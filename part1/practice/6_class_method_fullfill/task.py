...
test_assert(
    output_data == ['Создан объект Python. Количество 4. Осталось 6',
                    ...
                    'Теперь у объекта Python 11 единиц. На складе 3'], correct='Вывод в консоль верный', incorrect='Вывод в консоль НЕ верный')
...
try:
  python.fulfill(qnt='4')
  print('Метод fullfill класса Goods реализован не верно. В случае если передано не число, должно быть брошено исключение ValueError')
except ValueError:
  print('Метод fullfill класса Goods корректно ведет себя, если было передано не число')

try:
  python.fulfill(qnt=-2)
  print('Метод fullfill класса Goods реализован не верно. В случае если передано число меньше единицы, должно быть брошено исключение ValueError')
except ValueError:
